...
# Cargar configuración del vehículo desde la base de datos
vehicle_config = db.get_vehicle_config()
# today_record se cargará después de seleccionar la fecha
...
# Una Highlander Hybrid 2025 da aprox 35-36 MPG. Si es gasolina pura, unos 24 MPG.
    ...
    with col_save2:
        if st.button(f"💾 Guardar Registro del {fecha_label_btn}", type="primary", use_container_width=True):
            ...
            if db.save_daily_record(record_data, selected_date_str):
                st.success(f"✅ Registro del {fecha_label_btn} guardado exitosamente!")
                st.rerun()
            else:
    ...
    with tab2:
        ...
        if records:
            ...
            for record in records:
                with st.expander(f"📅 {record['date']} - Ganancia Neta: ${float(record['net_profit']):.2f}"):
                    col_h1, col_h2, col_h3 = st.columns(3)
                    col_h1.metric("Ingreso Bruto", f"${float(record['total_gross']):.2f}")
                    col_h2.metric("Gastos", f"${float(record['total_expenses']):.2f}")
                    col_h3.metric("Ganancia Neta", f"${float(record['net_profit']):.2f}")
                    
                    col_h4, col_h5 = st.columns(2)
                    col_h4.write(f"**Millas:** {float(record['miles_driven']):.1f} mi")
                    col_h5.write(f"**Combustible:** ${float(record['fuel_cost']):.2f}")
                    
                    if st.button(f"🗑️ Eliminar", key=f"delete_{record['id']}"):
                        db.delete_record(record['date'])
                        st.rerun()
        else:
...
# Mostrar sección de historial y estadísticas para Semanal y Mensual
    ...
    if view_option == "📆 Semanal":
        ...
        for i in range(7):
            ...
            record = db.get_record_by_date(date.isoformat())
            if record:
